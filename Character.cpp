...
void Character::victoryBoost(int& myStats, int& myStatInitVal)
{
    if( myStats < myStatInitVal ) { myStats = myStatInitVal; }  // a) restore to init if lower
    myStats *= 1.1;                                             // b) boost by 10%
    myStatInitVal = myStats;                                    // c) update initial value
}
